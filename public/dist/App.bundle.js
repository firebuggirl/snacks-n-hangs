...
//bling.js gives short form of querySelector and querySelectorAll
...
function autocomplete(input, latInput, lngInput) {
  ...
  var dropdown = new google.maps.places.Autocomplete(input); //part of Google API
  ...
  input.on('keydown', function (e) {
    //use .on() instead of .addEventListener() because we're using 'bling' shortcode
...
function ajaxHeart(e) {
  ...
  console.log(this);
  _axios2.default.post(this.action //'this' is the form tag..ie., heart..'action' is url we're psoting to
  ).then(function (res) {
    console.log(res.data);
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); //because 'this' form element has "heart" as a name attribute in storeCard.pug file, can access it as subproperty of 'this'
    console.log(isHearted);
    (0, _bling.$)('.heart-count').textContent = res.data.hearts.length; //res.data gives entire user, .hearts gives hearts array, and .length tells how many hearts in array
    if (isHearted) {
      _this.heart.classList.add('heart__button--float'); //add CSS animation/keyframe when heart i clicked
      setTimeout(function () {
        return _this.heart.classList.remove('heart__button--float');
      }, 2500); //remove after 2.5 secs
    }
  }).catch(console.error);
} //library for fetching AJAX
...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); //library for fetching AJAX
...
var mapOptions = { //see Gooogle Maps docs for more options
...
function loadPlaces(map) {
  ...
  //default coordinates
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng //hitting our own api and bringing back data
  ).then(function (res) {
    var places = res.data;
    //console.log(places);
    if (!places.length) {
    ...
    // create a bounds
    var bounds = new google.maps.LatLngBounds(); //zoom in to where markers are
    var infoWindow = new google.maps.InfoWindow(); //create Google info window

    var markers = places.map(function (place) {
      //map over places array and return a makrker for each
      var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
          placeLng = _place$location$coord[0],
          placeLat = _place$location$coord[1]; // MongoDB is lng/lat


      console.log(placeLng, placeLat);
      var position = { lat: placeLat, lng: placeLng }; //Google maps is lat/long
      ...
      var marker = new google.maps.Marker({ map: map, position: position }); //create marker
    ...
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        //'addListener' = GoogleMaps version of addEventListener
...
function makeMap(mapDiv) {
  //run makeMap on page load
  if (!mapDiv) return;
  // make our map
  var map = new google.maps.Map(mapDiv, mapOptions);
  loadPlaces(map); //then run loadPlaces

  var input = (0, _bling.$)('[name="geolocate"]'); //"$" is JS shorthand syntax imported from 'bling'
  var autocomplete = new google.maps.places.Autocomplete(input);
  autocomplete.addListener('place_changed', function () {
    //load new locations when user types diff location
...
//prevent XSS attacks...use anywhere you are setting HTML, like on lines 36 and 40!!!!
...
} //a Promise-based HTTP client for JavaScript which can be used in your front-end application and in your Node.js backend. send asynchronous HTTP request to REST endpoints and perform CRUD operations.
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data)); //prevent XSS attacks w/dompurify.sanitize
        return;
      }
      // tell them nothing came back
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results for ' + _this.value + '</div>'); ////prevent XSS attacks w/dompurify.sanitize
    }).catch(function (err) {
      console.error(err);
    });
  });
  ...
  searchInput.on('keyup', function (e) {
    ...
    var items = search.querySelectorAll('.search__result'); //look for all search results
    var next = void 0; // set to 'let' because there will be changes/updates
    if (e.keyCode === 40 && current) {
      //press down and one is already selected, then
      next = current.nextElementSibling || items[0]; //next one is going to be current or fall back to first item in array
    } else if (e.keyCode === 40) {
      next = items[0];
    } else if (e.keyCode === 38 && current) {
      //if pressing up, opposite of above
...
(0, _autocomplete2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lat'), (0, _bling.$)('#lng')); //Google maps does lat and lng the right way & MongoDb does it opposite
//Not jQuery, this is bling.js shorthand for doc.querySelectorAll, etc....
//

(0, _typeAhead2.default)((0, _bling.$)('.search'));

(0, _map2.default)((0, _bling.$)('#map')); //use bling.js to makeMap an id of map

var heartForms = (0, _bling.$$)('form.heart'); //"$$" is 'bling' shorthand for 'querySelectorAll'
